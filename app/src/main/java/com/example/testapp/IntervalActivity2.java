...
class TimerActivity2 extends Activity {
    ...
    private void startWorkTimer() {
        ...
        if (this.hasWorkBeenPaused) {
            ...
            this.workCountDownTimer = new CountDownTimer((((this.workMins * 60L) + this.workSecs) * 1000), 1000) {
                @Override
    ...
    private void startRestTimer() { // A method used to start the rest timer a.k.a the timer stars when it's time for you to do a lightweight exercise, for example
        ...
        if (this.hasRestBeenPaused) {
            ...
            this.restCountDownTimer = new CountDownTimer((((this.restMins * 60L) + this.restSecs) * 1000), 1000) {
                @Override
                public void onTick(long millisUntilFinished) {
                    // every second decrement the work seconds
    ...
    private void getViews() {  // This method gets all of the views from 'activity_main.xml' using the 'R' class
        ...
        this.thisActivity = findViewById(R.id.TimerActivity2);
    ...
    private void setTypeFacesToViews() {  // This method gets invoked if the user's android version is too low to handle XML fonts, it assigns the font faces manually
        Typeface tf = ResourcesCompat.getFont(getApplicationContext(), R.font.roboto);
...
//        this.createNotification();
    ...
    public void continueTimer(View view) {
        ...
        if (this.isWorkOn) {
            this.workCountDownTimer = new CountDownTimer((((this.pausedWorkMins * 60L) + this.pausedWorkSecs + 1) * 1000), 1000) {
                @Override
                public void onTick(long millisUntilFinished) {
                    decrementWork(trainingWorkQuantity);
                    updateWork();
            ...
            this.restCountDownTimer = new CountDownTimer((((this.pausedRestMins * 60L) + this.pausedRestSecs + 1) * 1000), 1000) {
